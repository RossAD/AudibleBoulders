...
var users = module.exports = promise.promisifyAll({
  ...
  getDashboardUsers: function (dashboardId, callback) {
    ...
    pool.query('SELECT github_handle, github_name, github_avatar, set_up, last_pulled_commit_sha1, last_pulled_commit_msg, commit_branch, signature_hash FROM users_dashboards INNER JOIN users ON users_dashboards.users_github_id=users.github_id WHERE dashboards_id=?', [dashboardId], function (err, results) {
